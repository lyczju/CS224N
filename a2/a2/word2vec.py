...
def naiveSoftmaxLossAndGradient(
    ...
    ### to integer overflow. 
    prob = softmax(outsideVectors @ centerWordVec)  # in shape (num words in vocab, )
    loss = -np.log(prob[outsideWordIdx])
    gradCenterVec = -outsideVectors[outsideWordIdx] + outsideVectors.T @ prob
    prob[outsideWordIdx] -= 1
    gradOutsideVecs = prob.reshape((-1, 1)) @ centerWordVec.reshape((1, -1))
...
def negSamplingLossAndGradient(
    ...
    u_K = outsideVectors[negSampleWordIndices]  # in shape (K, n)
    prob_o = u_o @ centerWordVec
    prob_neg = u_K @ centerWordVec
    loss = -np.log(sigmoid(prob_o)) - np.sum(np.log(sigmoid(-prob_neg)))
    coef_mat = np.diag([1 - sigmoid(-u_k @ centerWordVec) for u_k in u_K])  # in shape (K, K)
    gradCenterVec = -u_o * (1 - sigmoid(prob_o)) + np.sum(coef_mat @ u_K, axis=0)
    gradOutsideVecs = np.zeros_like(outsideVectors)
    gradOutsideVecs[outsideWordIdx] = -centerWordVec * (1 - sigmoid(prob_o))
    # for i in range(len(negSampleWordIndices)):
    #     gradOutsideVecs[negSampleWordIndices[i]] += centerWordVec * (1 - sigmoid(-prob_neg[i]))
    for i, p in enumerate(negSampleWordIndices):
        gradOutsideVecs[p] += centerWordVec * (1 - sigmoid(-prob_neg[i]))
...
def skipgram(currentCenterWord, windowSize, outsideWords, word2Ind,
    ...
    for w_k in outsideWords:
        loss_k, gradCenterVecs_k, gradOutsideVectors_k = word2vecLossAndGradient(v_c, word2Ind[w_k], outsideVectors, dataset)
        loss += loss_k
        gradCenterVecs[word2Ind[currentCenterWord]] += gradCenterVecs_k
        gradOutsideVectors += gradOutsideVectors_k
