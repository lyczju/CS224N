...
class PartialParse(object):
    ...
    def parse_step(self, transition):
        ...
        if transition == 'S':
            ...
            first_word = self.stack[len(self.stack) - 1]
            second_word = self.stack.pop(len(self.stack) - 2)
            self.dependencies.append((first_word, second_word))
        elif transition == 'RA':
            second_word = self.stack[len(self.stack) - 2]
            first_word = self.stack.pop(len(self.stack) - 1)
...
def minibatch_parse(sentences, model, batch_size):
    ...
    while len(unfinished_parses) > 0:
        ...
        batches_transitions = model.predict(batches)
        for batch, transition in zip(batches, batches_transitions):
            batch.parse_step(transition)
