...
class CharCorruptionDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        truncated_document = document if truncate_len >= len(document) else document[:truncate_len]
        truncate_len = len(truncated_document)
        ...
        # make sure that the length is picked _randomly_ (has a chance of being more or
        # less than 1/4 the length of the truncated document) for full credit.
        masked_content_len_avg = truncate_len / 4
        noise = random.uniform(-1/8, 1/8) * truncate_len
        masked_content_len = masked_content_len_avg + noise
